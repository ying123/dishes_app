...
import 'package:dishes_app/common/widgets/loading.dart';
...
//  }
  ...
  _initValue() {
      ...
      for (var item in _list) {
        if (item['title']==_model.personNum) {
          item['checked']=true;
        } else {
          item['checked']=false;
        }
  ...
  Widget build(BuildContext context) {
    if(_model==null){
      _model= Provider.of<OrderProvider>(context).getOrder;
      _p_mark=_model.remark??"";
      _p_num=_model.personNum??"";
      _initValue();
    }
    ...
    return Scaffold(
      ...
      body: FutureBuilder(
        builder: (context ,snapShot){
          return _init(width,_model);
        },
      ),
    );
  }
  _init(width,OrderModel order){
                ...
                Container(
                  ...
                  child: GestureDetector(
                    onTap: () async {
                      ...
                      await HttpUtil.getInstance().post(Api.updateOrder,data: {"personNum":_p_num,"remark":_p_mark,"orderId":order.orderId,"tableNo":order.tableNo}).then((res){
                        if(res["result"]){
                          Navigator.pop(context);
                        }
                      });
